...
def print_items(electives)
  electives.each.with_index { |val,index| puts "#{index} #{val}"}
end
...
def average(electives)
  electives..inject{ |sum, el| sum + el }.to_f / arr.size
end
